...
export function parseNKStrokes(csv: any[][], date: string): Stroke[] {
    ...
    while (index < csv.length) {
        ...
        if (stroke.catch && stroke.finish) {
            stroke.totalAngle = (-1 * stroke.catch) + stroke.finish;
        }
        if (stroke.slip && stroke.wash && stroke.totalAngle) {
            stroke.effectiveAngle = stroke.totalAngle - stroke.slip - stroke.wash;
        }
        if (stroke.catch && stroke.forceMaxDeg) {
            stroke.peakAfterCatch = stroke.catch - stroke.forceMaxDeg;
        }
        if (stroke.peakAfterCatch && stroke.slip) {
            stroke.peakAfterLoad = (stroke.catch + stroke.slip - stroke.forceMaxDeg);
        }
